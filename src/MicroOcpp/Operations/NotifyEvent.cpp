...
NotifyEvent::NotifyEvent(Model& model, int requestId, const Timestamp& generatedAt, bool tbc, int seqNo, const Vector<Variable*>& reportData)
        : MemoryManaged("v201.Operation.", "NotifyEvent"), model(model), requestId(requestId), generatedAt(generatedAt), tbc(tbc), seqNo(seqNo), reportData(reportData) {
